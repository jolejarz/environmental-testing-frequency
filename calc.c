...
void calculate (double f, void (*c2_r_p_set)(unsigned int, double, double, double, double*, double*, double*), double lambda, double c1, unsigned int n, unsigned int N, unsigned int n_sq, double phi, double *avg, double *stderror)
{
	...
	#pragma omp parallel for private(cost,time_infection_previous,time_sample_max,j,r,p,c2,time_infection,time_sample,infections) shared(cost_sum,cost_sum_squared)
	for (k=0; k<N; k++)
	{
		...
		cost+=c1/(double)T; // Add the surveillance cost per unit time.
	...
	*stderror = sqrt((cost_sum_squared/(double)N-(cost_sum/(double)N)*(cost_sum/(double)N))/(double)(N-1)); // Return the standard error.
